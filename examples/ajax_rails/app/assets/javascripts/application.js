...
var appendTask = function(task){
	...
	if (task.name.length !== 0) {
		if (!task.completed) {
			...
			$('#completed-items').appened(htmlTask);
		}
	}
}
...
$(function(){
	...
	$.ajax({
		...
		for (var i = 0; i < data.length; i++) {
		 ...
		 // $(task).each(function(index, task){ appendTask(task)});
			// STEP 10: For now use `console.log` in `.done()` to see the result is an array of javascript objects.
			// STEP 16: Rewrite the `.done()` method in step 10 so that it loops through the elements
			// in the ajax response object and passes each to the new append task function
	});
